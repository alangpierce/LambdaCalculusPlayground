...
const genType = (typeName, typeData) => {
    ...
    if (typeData.type === 'literal') {
        ...
        return genStruct(typeName, typeData.mixinClass, typeData.fields);
    } else if (typeData.type === 'union') {
        return genUnion(typeName, typeData.cases);
    }
};

const genStruct = (typeName, mixinClassName, fields) => {
    const {genLines, genComma} = fieldOperators(fields);
    const mixinClassCode = mixinClassName ? 'mixins.' + mixinClassName : 'null';
    return `\
export const ${typeName} = buildValueClass('${typeName}', ${mixinClassCode}, [${genComma((f) => `'${f}'`)}]);
`;
};
...
export const generateFlowTypes = (types) => {
    ...
    for (const key of Object.keys(types)) {
        if (types[key].type == 'struct') {
            structNames.add(key);
            if (types[key].mixinClass) {
                structMixins.push(types[key].mixinClass);
            }
        }
    }
...
import {
${structMixins.map((mixinName) => `    ${mixinName},`).join('\n')}
} from './types-mixins';
...
${genListLensTypes()}`;
    ...
    const genFlowType = (typeName, typeData) => {
        ...
        if (typeData.type === 'literal') {
            ...
            return genFlowStruct(typeName, typeData.mixinClass, typeData.fields);
        } else if (typeData.type === 'union') {
...
`;
    ...
    const genFlowStruct = (typeName, mixinClassName, fields) => {
        const {genLines, genComma} = fieldOperators(fields);
        const mixinCode = mixinClassName ? `extends ${mixinClassName} ` : '';
        return `\
declare export class ${typeName} ${mixinCode}{
...
export const generateMixinTemplates = (types) => {
    const structNames = [];
    for (const key of Object.keys(types)) {
        if (types[key].type == 'struct') {
...
${structNames.map((structName) => `\
export const ${structName}MixinTemplate = class {};\
`).join('\n')}
`;
};

export const generateFlowMixinTemplates = (types) => {
    const structNames = [];
    for (const key of Object.keys(types)) {
        if (types[key].type == 'struct') {
            structNames.push(key);
...
import {
${structNames.map((structName) => `    ${structName}`).join('\n')}
} from './types'

${structNames.map((structName) => `\
export class ${structName}MixinTemplate extends ${structName} {}\
`).join('\n')}
 `;
};